...
// High-precision timestamp for sub-microsecond timing
using Timestamp = std::chrono::time_point<std::chrono::high_resolution_clock>;
using Price = int64_t;  // Fixed-point arithmetic (multiply by 10000 for 4 decimal places)
using Quantity = uint64_t;
using OrderId = uint64_t;
using Symbol = std::array<char, 16>;  // Fixed-size for cache efficiency
...
struct alignas(64) MarketTrade {
    ...
    uint8_t padding[5];  // Explicit padding for alignment
...
// Market statistics for real-time analytics
...
// Simplified message structure without problematic union
struct alignas(64) MarketDataMessage {
    ...
    // Simplified: just store trade data (most common case)
