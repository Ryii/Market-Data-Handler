...
public:
    ...
    void stop() {
        ...
        if (data_generator_thread_.joinable()) data_generator_thread_.join();
...
private:
    ...
    nlohmann::json create_market_update() {
        ...
        // Get symbols from order book manager
        ...
        for (const auto& symbol_str : symbols) {
            ...
            if (book) {
                ...
                symbol_data["last_price"] = (symbol_data["bid_price"].get<double>() + 
                                           symbol_data["ask_price"].get<double>()) / 2.0;
    ...
    bool send_ws_frame(int client_fd, const std::string& data) {
        ...
        if (data.length() < 126) {
            ...
            // For simplicity, we don't handle very large frames
            return false;
        }
    ...
    void generate_market_data() {
        ...
        while (running_.load()) {
            for (size_t i = 0; i < symbols.size(); ++i) {
                ...
                base_prices[i] = new_price; // Update base price
